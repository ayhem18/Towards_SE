...
def convert_parquet_to_csv(input_file: str, batch_size: int = 10000) -> Path:
    ...
    is_first_batch = True
    for batch in tqdm(parquet_file.iter_batches(batch_size=batch_size),  desc="Converting Parquet to CSV"):
        df = batch.to_pandas()
...
if __name__ == '__main__':
    ...
    input_file = os.path.join(script_dir, "yellow_tripdata_2010-01.parquet")

    output_file = convert_parquet_to_csv(input_file, 100000) 

    df = pd.read_csv(output_file, nrows=10000) # just enough to get the dtypes 
